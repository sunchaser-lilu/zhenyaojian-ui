...
export default {
  ...
  methods: {
    ...
    renderMenuItem: function (h, menu) {
      ...
      const target = menu.meta.target || '_blank'
      const hasRemoteUrl = httpReg.test(menu.path)
      if (hasRemoteUrl) {
        tag = 'a'
        config = { attrs: { style: 'overflow:hidden;white-space:normal;text-overflow:clip;', href: menu.path, target: target } }
      }
      if (menu.meta && menu.meta.link) {
        tag = 'a'
        config = { attrs: { style: 'overflow:hidden;white-space:normal;text-overflow:clip;', href: menu.meta.link, target: target } }
      }
      return h(
